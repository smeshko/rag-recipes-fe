...
/* Harness for the controlled contract (2.1 migration): the field owns no
   text state — callers do. */
...
describe("SearchInput", () => {
  ...
  it("follows a customised placeholder", () => {
    render(
      <ControlledSearch placeholder="Search this book" onSubmit={vi.fn()} />,
    );
    expect(
  ...
  it("lets the caller override the accessible name explicitly", () => {
    render(
      <ControlledSearch label="Search your cookbooks" onSubmit={vi.fn()} />,
    );
    expect(
  ...
  it("keeps the magnifier out of the accessibility tree", () => {
    const { container } = render(<ControlledSearch onSubmit={vi.fn()} />);
  ...
  it("renders and edits a caller-seeded value", async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn();
    render(<ControlledSearch initial="scones" onSubmit={onSubmit} />);
